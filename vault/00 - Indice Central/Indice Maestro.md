# 🗺️ Mapa Central de Contenidos (MOC Maestro)

> *"Conecta conceptos, domina los principios fundamentales."*

Este es el nodo raíz de la bóveda de conocimiento de 2º curso del Grado en Ingeniería Aeroespacial.

---

## 📚 Asignaturas Activas (Primer Cuatrimestre)

### 🌊 [[01 - Fluid Mechanics/Mecanica de Fluidos MOC|Mecánica de Fluidos]]
* **Temas principales:** Estática de fluidos, Cinemática, Ecuaciones integrales y diferenciales (Navier-Stokes), Análisis dimensional, Flujo potencial, Capa límite y Compresibilidad.
* **Cuaderno NotebookLM asociado:** `3080c1f2-5689-4a39-90c3-091d58f39684`
* **Página Web:** `subjects/fluid-mechanics/`

### 🔬 [[02 - Aerospace Materials I/Materiales Aeroespaciales I MOC|Materiales Aeroespaciales I]]
* **Temas principales:** Estructuras cristalinas, Diagramas de fases binarios, Aleaciones de aluminio, titanio y superaleaciones de níquel, Tratamientos térmicos, Ensayos mecánicos (tracción, fatiga, fluencia, tenacidad).
* **Cuaderno NotebookLM asociado:** `9b324478-69e9-482c-813c-5709ec031820`
* **Página Web:** `subjects/aerospace-materials-1/`

### 🏗️ [[03 - Engineering Mechanics/Mecanica de Estructuras MOC|Mecánica Aplicada a la Ingeniería Aeroespacial]]
* **Temas principales:** Cinemática y dinámica del punto material, movimiento relativo, oscilador armónico, problema orbital de Kepler, cinemática y dinámica 3D del sólido rígido, geometría de masas, ecuaciones de Euler y mecánica de vuelo atmosférico.
* **Cuaderno NotebookLM asociado:** `473546c3-3716-4426-b0c4-58de530f91c8`
* **Página Web:** `subjects/engineering-mechanics/`

### 📐 [[04 - Advanced Maths/Matematicas Avanzadas MOC|Matemáticas Avanzadas]]
* **Temas principales:** Campos vectoriales y teoremas integrales (Green, Stokes, Divergencia), Variable compleja y residuos, Series y transformadas de Fourier, Ecuaciones en derivadas parciales (onda, calor, Laplace).
* **Cuaderno NotebookLM asociado:** `c27033c3-5a64-403f-a517-5847831aabcb`
* **Página Web:** `subjects/advanced-maths/`

### 📊 [[05 - Business Management/Gestion de Empresas MOC|Organización de Empresas]]
* **Temas principales:** Contabilidad financiera y analítica, Evaluación y selección de proyectos de inversión (VAN, TIR), Gestión de producción e inventarios, Planificación estratégica y organización de empresas aeroespaciales.
* **Cuaderno NotebookLM asociado:** `e691ea81-0acf-4032-98d1-b9e5a7b93d70`
* **Página Web:** `subjects/business-management/`

---

## 🛠️ Herramientas y Plantillas
* `[[Templates/Plantilla Concepto Teorico|Plantilla: Concepto Teórico]]`
* `[[Templates/Plantilla Problema de Examen|Plantilla: Problema de Examen Resuelto]]`
* `[[Templates/Plantilla Formulario Resumen|Plantilla: Formulario y Prontuario]]`
